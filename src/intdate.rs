...
impl IntDate {
    ...
    fn month(&self, year: u16, mut days: u16) -> (&str, u8, u8) {
        ...
        if days <= 0 {
            ...
            month = last_month;
            month_num -= 1;
            month_num += 12 * (month_num == 0) as u8;
        }
    ...
    fn weekday(&self, mut year: u16, month: u8, mut day: u8) -> usize {
        ...
        let mut output = ((year + (year as f32 / 4.).floor() as u16
            - (year as f32 / 100.).floor() as u16
            + (year as f32 / 400.).floor() as u16
            + t[(month - 1) as usize]
            + day as u16
            + (year == 1_900 && self.is_stupid) as u16
            - (year == 1_900 && actual_year > 1_900 && self.is_stupid) as u16)
            % 7);
